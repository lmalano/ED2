...
alarma equ d'1'		;bandera que me indica si esta activada o no la alarma
desplazamiento equ d'2'	;bandera que me indica si debo hacer un desplazamiento del texto del panel
cargarChar equ d'3'	;bandera que me habilita a cargar otro caracter
...
confAlarma equ d'4'	;bandera para indicar que se debe configurar una nueva alarma y se muestra el panel de configuracion de reloj, pero los valores se guardan en otros registros
textoAlarma equ d'5'	;bandera para mostrar el texto animado por ser la hora a la que se configuro la alarma
...
banderasDisplayAux equ 0x27	    ;guardo temporalmente la bandera activa para luego volver a la actividad que se estaba realizando
punteroDesplazam equ 0x28	    ;puntero para indicar desde donde empiezo a mostrar el texto del panel
mult equ 0x29
...
char22 equ 0x5E
char23 equ 0x5F
minutoAlarmaL equ 0x60			;registros para hora de alarma (en binario puro)
minutoAlarmaH equ 0x61
horaAlarmaL equ 0x62
horaAlarmaH equ 0x63
contChar equ 0x64			;limitador de caracteres ingresados
contCharRef equ 0x65			;referencia de contChar
contCharRefAux equ 0x66
...
COMIENZO
    ...
    clrf horaLAux
    clrf horaHAux
    movlw b'00001000'			    ;la unica bandera activa desde el inicio es la que me posibilita cargar caracteres
    movwf banderas
    movlw b'00000001'
    ...
    call INICCONFHORA			    ;inicializo el panel de configuracion del reloj y alarma
    movlw "-"
    movwf unidadADC
    movlw "."
    movwf separadorTemp
    ...
    movlw 0x5B
    movwf punteroDesplazam			    ;inicializo el puntero del panel de texto
    call INICPANEL				    ;inicializo el panel de texto
    movlw d'23'
    movwf contChar
    movlw d'18'
    movwf contCharRef
...
TIMER			    ;rutina de interrupcion de timer
    ...
    ;movlw d'20'
    ;movwf mult
    btfsc banderasDisplay,reloj		   
    goto MOSTRARHORA				;se muestra el reloj
    btfsc banderasDisplay,configuracion
    goto MOSTRARCONFIG				;se muestra la configuracion del reloj
    btfsc banderasDisplay,temperatura
    goto MOSTRARTEMP				;se muestra la temperatura
    btfsc banderasDisplay,textoConf
    goto MOSTRARENTRADA				;se muestra el editor de texto
    btfsc banderasDisplay,confAlarma
    goto MOSTRARCONFALARMA			;se muestra la configuracion de la hora de la alarma
    bsf banderasDisplay,textoAlarma
    goto MOSTRARALARMA				;dispara la alarma y se muestra el texto correspondiente
TIMERCONTROL
...
SENSARTEMP			;si decena de segundos es impar y el rersto de los submultiplos es cero, manda a sensar la temperatura con el ADC
    ...
    goto CONTROLDESPLAZ			;no
    btfsc unmilAux,0		;si. ahora, es impar la decena de segundos? En caso negativo se salta a CONTROLDESPLAZ
...
FINCTRLDESP
    movf unidadAux,0
    btfsc STATUS,Z
    call MODIFPUNTALARMA
ALARMA
    btfss banderas,alarma	;está activa la alarma?
    goto FINCUENTA		;no
    movf unidadAux,0		;si, se verifica se llego a la hora indicada para disparar
    btfsc STATUS,Z		;es cero la centecima de segundos?
    movf decenaAux,0		
    btfsc STATUS,Z		;es cero la decima de segundos?
    movf centenaAux,0
    ...
    btfss STATUS,Z
    goto FINCUENTA
    movf minutoAlarmaL,0
    subwf minutoLAux,0
    btfss STATUS,Z
    goto FINCUENTA
    movf minutoAlarmaH,0
    subwf minutoHAux,0
    btfss STATUS,Z
    goto FINCUENTA
    movf horaAlarmaL,0
    ...
    movf horaAlarmaH,0
    subwf horaHAux,0
    btfss STATUS,Z			    ;es hora de disparar la alarma?
    goto FINCUENTA			    ;aun no	
    movf banderasDisplay,0		    ;si, se guarda una copia de la bandera activa para volver luego, y se comienza a mostrar el mensaje de alarma
    movwf banderasDisplayAux
    clrf banderasDisplay
    bsf banderasDisplay,textoAlarma	    ;muestro mensaje
    bcf banderas,alarma			    ;dejo de verificar si debe disparar la alarma    
FINCUENTA
    bcf INTCON,T0IF
    call RECCONTEXTO
    retfie
...
;-------------------------------------INICIALIZACION DEL PANEL DE MENSAJES---------------------------------------;
...
;-----------------------------------------------RUTINAS DE CONTROL3-------------------------------------;
MODIFPUNTALARMA
    btfss banderas,desplazamiento
    return
    decf punteroDesplazam,1
    decfsz contCharRef,1
    return
    movlw 0x5B
    movwf punteroDesplazam
    movf contCharRefAux,0
    movwf contCharRef
    return
...
GUARDARHORA				    ;confirmo la hora elegida, la codifico a binario natural y reemplazo esos valores en los registros del reloj
    movlw d'48'	
    subwf confMinutoL,0
    ...
    call INICCONFHORA			    ;reseteo el panel de configuracion de reloj/alarma
    goto FINTECLADO
    
GUARDARTEXTO				    ;se cambian las banderas para seguir mostrando el reloj, ya que el texto ya queda guardado al ingresarse. Ademas se define un limite para el desplazamiento del texto
    bcf banderasDisplay,textoConf
    bsf banderasDisplay,reloj
    movf contChar,0
    subwf contCharRef,0
    btfsc STATUS,C			    ;la cantidad de caracteres ingresados es mayor a 5?
    goto ACTIVARDESPL			    ;si, se activa el desplazamiento
    goto DESACTDESPL			    ;no, se desactiva

    
GUARDARALARMA
    movlw d'48'	
    subwf confMinutoL,0
    movwf minutoAlarmaL			    ;confirmo la unidad de minutos y lo guardo como binario puro
    movlw d'48'
    subwf confMinutoH,0
    movwf minutoAlarmaH			    ;confirmo la decena de minutos y lo guardo como binario puro
    movlw d'48'
    subwf confHoraL,0
    movwf horaAlarmaL			    ;confirmo la unidad de horas y lo guardo como binario puro
    movlw d'48'
    subwf confHoraH,0
    movwf horaAlarmaH			    ;cofirmo la unidad de minutos y lo guardo como binario puro
    clrf banderasDisplay
    bsf banderasDisplay,reloj		    ;ahora doy la orden para mostrar el reloj nuevamente
    bsf banderas,alarma			    ;activo la alarma
    call INICCONFHORA			    ;reseteo el panel de configuracion de reloj/alarma
    goto FINTECLADO
    
ACTIVARDESPL
    movwf contCharRef
    movwf contCharRefAux
    incf contCharRef
    incf contCharRefAux			    ;incrementamos una unidad cada contador para no reiniciar el puntero en el ultimo caracter y asi poder mostrarlo
    bsf banderas,desplazamiento
    goto FINTECLADO
    
DESACTDESPL
    bcf banderas,desplazamiento
    goto FINTECLADO
